...
//    echo "$query<br>";
 ...
 /**
  ...
  public function alterar($where = null, $dados = null) {
    if (!is_null($where) && !is_null($dados)) {
        $valores = [];
        ...
        $query = "UPDATE " . $this->tabela . " SET " . $valores . " WHERE " . $where[0] . " = :whereValue";
        ...
        try {
            ...
            $stmt->bindParam(':whereValue', $where[1]);

            $stmt->execute();
            return true;
        } catch (PDOException $e) {
            $mensagem = "Db class alterar() = " . $e->getMessage();
            file_put_contents("erro.log", $mensagem, FILE_APPEND);
            return false;
        }
    } else {
